...
class StaffService {
  ...
  Future<Map<String, dynamic>> getBookingDetail(int bookingId) async {
    try {
      ...
      final response = await dio.get(
        '${ApiConstants.apiStaff}/booking/$bookingId',
      );
      if (response.statusCode == 200) {
        final data = response.data as Map<String, dynamic>;
        return {'success': data['success'], 'data': data['data']};
      }
      return {'success': false, 'message': 'Lỗi tải chi tiết đơn'};
    } catch (e) {
      return {'success': false, 'message': 'Lỗi: ${e.toString()}'};
    }
  }
  ...
  Future<Map<String, dynamic>> cancelBooking(int bookingId) async {
    try {
      final dio = getHttpClient();
      final response = await dio.post(
        '${ApiConstants.apiStaff}/booking/cancel',
        options: Options(headers: jsonHeaders()),
      ...
      return {'success': false, 'message': 'Lỗi hủy đơn'};
    } catch (e) {
      return {'success': false, 'message': 'Lỗi: ${e.toString()}'};
    }
  }
